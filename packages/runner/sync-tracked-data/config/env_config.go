...
type Config struct {
	...
	TrackingPostgresDb struct {
		Host            string `env:"DB_TRACKING_HOST,required"`
		Port            int    `env:"DB_TRACKING_PORT,required"`
		Pwd             string `env:"DB_TRACKING_PWD,required,unset"`
		Name            string `env:"DB_TRACKING_NAME,required"`
		User            string `env:"DB_TRACKING_USER,required"`
		MaxConn         int    `env:"DB_TRACKING_MAX_CONN"`
		MaxIdleConn     int    `env:"DB_TRACKING_MAX_IDLE_CONN"`
		ConnMaxLifetime int    `env:"DB_TRACKING_CONN_MAX_LIFETIME"`
	}

...
func (c currencyService) GetCurrencyRatesECB() {
	...
	for _, currency := range envelope.Cube.Cube.Currencies {
		if currency.Currency == "USD" {
			...
			err := c.repositories.CommonRepositories.CurrencyRateRepository.SaveCurrencyRate("EUR", usdToEurRate, date, "European Central Bank")
			if err != nil {
				tracing.TraceErr(span, err)
	...
	for _, currency := range envelope.Cube.Cube.Currencies {
		if currency.Currency != "USD" {
			...
			err := c.repositories.CommonRepositories.CurrencyRateRepository.SaveCurrencyRate(currency.Currency, usdToCurrency, date, "European Central Bank")
			if err != nil {
				tracing.TraceErr(span, err)
				c.log.Errorf("Error saving currency rate: %s", err.Error())
			}

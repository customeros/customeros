...
type SocialReadRepository interface {
	GetDuplicatedSocialsForEntityType(ctx context.Context, linkedEntityNodeLabel string, delayMinutes, limit int) ([]TenantSocialIdAndEntityId, error)
}
...
func (r *socialReadRepository) GetDuplicatedSocialsForEntityType(ctx context.Context, linkedEntityNodeLabel string, delayMinutes, limit int) ([]TenantSocialIdAndEntityId, error) {
	span, ctx := opentracing.StartSpanFromContext(ctx, "SocialReadRepository.GetDuplicatedSocialsForEntityType")
	defer span.Finish()
	span.LogFields(log.String("linkedEntityNodeLabel", linkedEntityNodeLabel), log.Int("delayMinutes", delayMinutes), log.Int("limit", limit))

	cypher := fmt.Sprintf(`MATCH (t:Tenant)--(e:%s)-[:HAS]->(s:Social)
					WHERE s.updatedAt < datetime() - duration({minutes: $delayMinutes})

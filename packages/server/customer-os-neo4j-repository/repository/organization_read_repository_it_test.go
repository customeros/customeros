...
func TestOrganizationReadRepository_GetOrganizationsForInvoicing_1_SLI_Monthly_V1(t *testing.T) {
	...
	contractId := neo4jtest.CreateContractForOrganization(ctx, driver, tenantName, organization1Id, entity.ContractEntity{})
...
func TestOrganizationReadRepository_GetOrganizationsForInvoicing_1_SLI_Monthly_V2(t *testing.T) {
	ctx := context.Background()
	defer tearDownTestCase(ctx)(t)
...
func assertNoOrganization(t *testing.T, ctx context.Context, invoiceDate time.Time) {
	organizationsForInvoicing, err := repositories.OrganizationReadRepository.GetOrganizationsForInvoicing(ctx, invoiceDate)
	if err != nil {
		t.Fatal(err)
	}

	require.Equal(t, 0, len(organizationsForInvoicing))
}

func assertInvoiceForOrganization(t *testing.T, ctx context.Context, invoiceDate time.Time, tenant, organizationId string) {
	organizationsForInvoicing, err := repositories.OrganizationReadRepository.GetOrganizationsForInvoicing(ctx, invoiceDate)
	if err != nil {
		t.Fatal(err)
	}

	require.Equal(t, 1, len(organizationsForInvoicing))

...
func OnWebhookEventCreated(ctx context.Context, dependencies *model.DependencyContainer, input any) error {
	...
	case data_fields.MeetingSummaryEvent{}.Type():
		...
		err = handlers.HandleMeetingSummaryEvent(ctx, dependencies.CommonServices, eventName, eventData)
		if err != nil {
			tracing.TraceErr(span, err)
			return err
		}
	...
	case data_fields.WebsiteVisitEvent{}.Type():
		...
		err = handlers.HandleWebsiteVisitorEvent(ctx, dependencies, eventName, eventData)
		if err != nil {
			tracing.TraceErr(span, err)
			return err
		}

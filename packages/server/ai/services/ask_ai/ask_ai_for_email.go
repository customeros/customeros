...
func (s *aiService) AskAIForEmail(ctx context.Context, message *pb.AskAI) (*data_fields.EmailResponse, error) {
	...
	// defaults
	message.RequestType = pb.AIRequestType_AI_REQUEST_EMAIL
...
%s`,
	...
	if lastError != nil {
		err := fmt.Errorf("email parsing failed after %d attempts: %w", *message.Retries, lastError)
		spans.TraceError(err)
		return nil, err
	}

	return nil, fmt.Errorf("email parsing failed after %d attempts", *message.Retries)
}

...
func (s *aiService) AskAIForIndustryCode(ctx context.Context, message *pb.AskAI) (*data_fields.IndustryCode, error) {
	...
	// defaults
	message.RequestType = pb.AIRequestType_AI_REQUEST_INDUSTRY_CODE
...
%s`,
	...
	if lastError != nil {
		err := fmt.Errorf("industry code identification failed after %d attempts: %w", *message.Retries, lastError)
		spans.TraceError(err)
		return nil, err
	}

	return nil, fmt.Errorf("industry code identification failed after %d attempts", *message.Retries)
}

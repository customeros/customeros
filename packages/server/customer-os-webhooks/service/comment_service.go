...
func (s *commentService) syncComment(ctx context.Context, syncMutex *sync.Mutex, commentInput model.CommentData, syncDate time.Time) SyncStatus {
	...
	if !failedSync {
		...
		request := commentpb.UpsertCommentGrpcRequest{
			...
			CreatedAt:   timestamppb.New(utils.TimePtrFirstNonNilNillableAsAny(commentInput.CreatedAt, utils.NowPtr()).(time.Time)),
			UpdatedAt:   timestamppb.New(utils.TimePtrFirstNonNilNillableAsAny(commentInput.UpdatedAt, utils.NowPtr()).(time.Time)),

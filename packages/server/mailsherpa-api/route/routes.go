...
func validateEmail(ctx context.Context, r *gin.Engine, services *service.Services, cfg *config.Config, l logger.Logger) {
	r.POST("/validateEmail",
		...
		security.ApiKeyCheckerHTTP(services.PostgresRepositories.TenantWebhookApiKeyRepository, services.PostgresRepositories.AppKeyRepository, security.VALIDATION_API, security.WithCache(services.Cache)),
		func(c *gin.Context) {
			...
			emailValidationData, err := services.VerifyService.ValidateEmail(ctx, request.Email)
			if err != nil {

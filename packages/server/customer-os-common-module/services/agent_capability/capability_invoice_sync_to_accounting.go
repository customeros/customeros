...
func (c *SyncInvoiceToAccountingCapability) Execute(ctx context.Context, executionContainer interfaces.TypedExecutionContainer[SyncInvoiceToAccountingInput, SyncInvoiceToAccountingConfig]) (enum.CapabilityExecutionStatus, SyncInvoiceToAccountingOutput, error) {
	spans, ctx := telemetry.StartServiceSpan(ctx, "SyncInvoiceToAccountingCapability.Execute")
	defer spans.Finish()
	...
	if invoiceEntity.DryRun {
		spans.LogKV("skip", "Dry run")
		return enum.CapabilityExecutionCompleted, result, nil
	}
	if invoiceEntity.TotalAmount == 0 {
		spans.LogKV("skip", "Total amount is 0")
		return enum.CapabilityExecutionCompleted, result, nil
	}
	if invoiceEntity.TotalAmount <= 0 {
		spans.LogKV("skip", "Total amount is negative")
		return enum.CapabilityExecutionCompleted, result, nil
	}
	...
	if invoiceEntity.Status == neo4jenum.InvoiceStatusPaid && invoiceEntity.QuickbooksPaidPaymentId == "" {
		...
		if executionContainer.ConfigData.AccountingMethodAccrual.Value && invoiceEntity.QuickbooksJournalEntryId != "" {
			...
			if invoiceEntity.QuickbooksPaymentId != "" {
				...
				err = c.quickbooksService.ZeroPaymentLinkingJournalEntryToInvoice(ctx, invoiceEntity.QuickbooksPaymentId, contractEntity.QuickbooksCustomerId, invoiceEntity.QuickbooksJournalEntryId)
				if err != nil {
					spans.TraceError(err)
					return enum.CapabilityExecutionRetry, result, err
				}
...
func (c *SyncInvoiceToAccountingCapability) syncInvoiceToQuickbooks(ctx context.Context, invoice neo4jentity.InvoiceEntity) error {
	spans, ctx := telemetry.StartServiceSpan(ctx, "SyncInvoiceToAccountingCapability.syncInvoiceToQuickbooks")
	defer spans.Finish()
	tenant := common.GetTenantFromContext(ctx)
	spans.TagEntity(invoice.Id)
	...
	if invoiceLines == nil || len(*invoiceLines) == 0 {
		spans.LogKV("skip", "Invoice lines not found")
		return nil
	}
	...
	for _, invoiceLine := range *invoiceLines {
		if invoiceLine.SkuId == "" {
			spans.LogKV("skip", fmt.Sprintf("SkuId not found for invoice line %s", invoiceLine.Id))
			return nil
		}
		...
		if sku.QuickbooksId == "" {
			...
			if err != nil {
				spans.TraceError(errors.Wrap(err, "error saving product to quickbooks"))
				return err
			}
			...
			if err != nil {
				spans.TraceError(errors.Wrap(err, "error saving sku in db"))
				return err
			}
		}
	}
...
func (c *SyncInvoiceToAccountingCapability) syncInvoiceToQuickbooksJournalEntry(ctx context.Context, invoice neo4jentity.InvoiceEntity, arIncomeAccountName string) error {
	spans, ctx := telemetry.StartServiceSpan(ctx, "SyncInvoiceToAccountingCapability.syncInvoiceToQuickbooksJournalEntry")
	defer spans.Finish()
	spans.TagEntity(invoice.Id)
	tenant := common.GetTenantFromContext(ctx)

	spans.LogKV("invoiceId", invoice.Id, "arIncomeAccountName", arIncomeAccountName)
	quickbooksSettingsEntity, err := c.postgresRepositories.QuickbooksSettingsRepository.Get(ctx, tenant)
	if err != nil {
		spans.TraceError(err)
		return err
...
func (c *SyncInvoiceToAccountingCapability) syncInvoiceToQuickbooksJournalEntryReverse(ctx context.Context, invoice neo4jentity.InvoiceEntity, arIncomeAccountName string) error {
	spans, ctx := telemetry.StartServiceSpan(ctx, "SyncInvoiceToAccountingCapability.syncInvoiceToQuickbooksJournalEntryReverse")
	defer spans.Finish()
	spans.TagEntity(invoice.Id)
...
func (c *SyncInvoiceToAccountingCapability) syncPaymentLinkingJournalEntryToInvoice(ctx context.Context, invoice neo4jentity.InvoiceEntity) error {
	spans, ctx := telemetry.StartServiceSpan(ctx, "SyncInvoiceToAccountingCapability.syncPaymentLinkingJournalEntryToInvoice")
	defer spans.Finish()
	spans.TagEntity(invoice.Id)
	...
	if err != nil {
		spans.TraceError(errors.Wrap(err, "error saving payment linking journal entry to invoice"))
		return err
	}
...
func (c *SyncInvoiceToAccountingCapability) syncPaidInvoiceToQuickbooks(ctx context.Context, invoice neo4jentity.InvoiceEntity, paymentIncomeAccountName string) error {
	spans, ctx := telemetry.StartServiceSpan(ctx, "SyncInvoiceToAccountingCapability.syncPaidInvoiceToQuickbooks")
	defer spans.Finish()
	spans.TagEntity(invoice.Id)
...
func (c *SyncInvoiceToAccountingCapability) syncVoidInvoiceToQuickbooks(ctx context.Context, invoice neo4jentity.InvoiceEntity) error {
	spans, ctx := telemetry.StartServiceSpan(ctx, "SyncInvoiceToAccountingCapability.syncVoidInvoiceToQuickbooks")
	defer spans.Finish()
	spans.TagEntity(invoice.Id)

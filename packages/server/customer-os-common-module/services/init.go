...
func InitCommonServices(
	...
	events, err := events.NewEventsService(config.RabbitMQConfig.Url, log)
	if err != nil {
	...
	common := CommonServices{
		...
		AIService:                  ai.NewAIService(&config.ExternalServices.AnthropicConfig),
		AttachmentService:          attachment.NewAttachmentService(neo4jRepositories),
		AzureService:               azure.NewAzureService(&config.AzureOAuthConfig, postgresRepositories, neo4jRepositories),
		CloudflareService:          cloudflare.NewCloudflareService(log, &config.ExternalServices.CloudflareConfig, postgresRepositories),
		...
		EnrichmentService:          enrichment.NewEnrichmentService(log, &config.ExternalServices, postgresRepositories),
		Events:                     events,
		ExternalSystemService:      externalsystem.NewExternalSystemService(log, neo4jRepositories, events),
		GoogleService:              google.NewGoogleService(&config.GoogleOAuthConfig, postgresRepositories, neo4jRepositories),
		...
		NamecheapService:           namecheap.NewNamecheapService(&config.ExternalServices.NamecheapConfig, postgresRepositories),
		NovuService:                novu.NewNovuService(config.ExternalServices.NovuCofig.ApiKey),
		OpenSRSService:             opensrs.NewOpenSRSService(log, &config.ExternalServices.OpenSRSConfig, postgresRepositories),
		PhoneNumberService:         phone_number.NewPhoneNumberService(neo4jRepositories, events),
		PostmarkService:            postmark.NewPostmarkService(&config.ExternalServices.PostmarkConfig, postgresRepositories),
	...
	common.FileService = files.NewFileService(
		log,
		&config.InternalServices.FileStoreConfig,
		neo4jRepositories,
	...
	sli := sli.NewServiceLineItemService(
		log,
		events,
		neo4jRepositories,
		nil, // contract
	...
	mailstack := mailstack.NewMailstackService(
		&config.ExternalServices.StripeConfig,
		events,
		postgresRepositories,
	...
	location := location.NewLocationService(
		...
		postgresRepositories,
		events,
		&config.ExternalServices.AnthropicPrompts,
		common.AIService,
		nil, // contact
		nil, // org
	)
	...
	sli.SetContractService(contract)
	if !sli.IsInitialized() {
		log.Fatalf("Common ServiceLineItem Service not initialized")
	}
	common.ServiceLineItemService = sli

	invoice.SetContractService(contract)
	invoice.SetServiceLineItemService(sli)

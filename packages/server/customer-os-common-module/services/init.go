...
func InitCommonServices(
	...
	slackImpl := slack.NewSlackService(log, postgresRepositories, warehouseRepositories)
	...
	quickbooksImpl := quickbooks.NewQuickbooksService(&cfg.External.QuickbooksConfig, postgresRepositories, warehouseRepositories)
	webhookImpl := webhook.NewWebhookService(log, postgresRepositories)
	webscrapeImpl := webscraper.NewWebscraperService(&cfg.External.JinaConfig, postgresRepositories, warehouseRepositories, aiImpl)
	...
	crustDataImpl := enrichment.NewCrustDataService(log, &cfg.External.CrustDataConfig, postgresRepositories, warehouseRepositories)
	...
	embeddingImpl := embedding.NewEmbeddingService(&cfg.External.JinaConfig, aiImpl, opensearchImpl, warehouseRepositories)
	...
	orgImpl := organization.NewOrganizationService(log, natsConn, postgresRepositories, neo4jRepositories, eventsImpl, domainImpl, industryImpl, socialImpl, userImpl, currencyImpl, emailImpl)
	...
	enrichmentImpl := enrichment.NewEnrichmentService(log, &cfg.External, cacheImpl, eventsImpl, postgresRepositories, warehouseRepositories, neo4jRepositories, contactImpl, domainImpl, locationImpl, orgImpl, socialImpl, globalContactImpl)
	verifyImpl := verify.NewVerifyService(log, postgresRepositories, warehouseRepositories, cfg, enrichmentImpl)

...
func (s *fileService) GetById(ctx context.Context, id string) (*interfaces.File, error) {
	...
	return MapAttachmentResponseToFileEntity(attachment), nil
}
...
func (s *fileService) UploadSingleFile(ctx context.Context, basePath, fileId string, multipartFileHeader *multipart.FileHeader, cdnUpload bool) (*interfaces.File, error) {
	...
	if s.cfg.CloudflareImageUploadApiKey != "" && s.cfg.CloudflareImageUploadAccountId != "" && s.cfg.CloudflareImageUploadSignKey != "" &&
		...
		uploadedFileToCdn, err := cloudflareApi.UploadImage(context.Background(), cloudflare.AccountIdentifier(s.cfg.CloudflareImageUploadAccountId), cloudflare.UploadImageParams{
			File:              readCloser,
			Name:              fileId,
		...
		attachmentEntity.CdnUrl = generateSignedURL(uploadedFileToCdn.Variants[0], s.cfg.CloudflareImageUploadSignKey)
	}
	...
	return MapAttachmentResponseToFileEntity(created), nil
}

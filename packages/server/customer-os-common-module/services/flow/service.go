...
func (s *flowService) FlowMerge(ctx context.Context, tx *neo4j.ManagedTransaction, input *neo4jentity.FlowEntity) (*neo4jentity.FlowEntity, error) {
	...
	flowEntity, err := utils.ExecuteWriteInTransaction(ctx, s.neo4j.Neo4jDriver, s.neo4j.Database, tx, func(tx neo4j.ManagedTransaction) (any, error) {
		...
		err := s.neo4j.FlowActionWriteRepository.DeleteForFlow(ctx, &tx, toStore.Id)
		if err != nil {
			return nil, err
		}
			...
			for _, v := range nodesMap {
				...
				if v["data"] != nil {
					...
					if e.Data.Action == neo4jentity.FlowActionTypeFlowStart ||
						...
						storedNode, err := s.neo4j.FlowActionWriteRepository.Merge(ctx, &tx, &e)
						if err != nil {
							return nil, err
						}
						...
						_, err = s.neo4j.FlowActionWriteRepository.Merge(ctx, &tx, stored)
						if err != nil {
							return nil, err
						}
...
func (s *flowService) FlowOn(ctx context.Context, id string) (*neo4jentity.FlowEntity, error) {
	...
	_, err = utils.ExecuteWriteInTransactionWithPostCommitActions(ctx, s.neo4j.Neo4jDriver, s.neo4j.Database, nil, func(txWithPostCommit *utils.TxWithPostCommit) (any, error) {
		node, err = s.neo4j.FlowWriteRepository.Merge(ctx, txWithPostCommit.Tx, flow)
		if err != nil {
			return nil, err
		}

		participantIdsUpdated, err := s.neo4j.FlowParticipantWriteRepository.MarkReadyContactsAsScheduled(ctx, txWithPostCommit.Tx, flow.Id)
		if err != nil {
			return nil, err
		}
...
func (s *flowService) FlowParticipantAdd(ctx context.Context, flowId, entityId string, entityType model.EntityType) (*neo4jentity.FlowParticipantEntity, error) {
	...
	if identified == nil {
		...
		if entityType == model.CONTACT {
			contactNode, err := s.neo4j.ContactReadRepository.GetContact(ctx, tenant, entityId)
			if err != nil {
		...
		e, err := utils.ExecuteWriteInTransactionWithPostCommitActions(ctx, s.neo4j.Neo4jDriver, s.neo4j.Database, nil, func(txWithPostCommit *utils.TxWithPostCommit) (any, error) {
			...
			toStore.Id, err = s.neo4j.CommonReadRepository.GenerateId(ctx, common.GetTenantFromContext(ctx), model.NodeLabelFlowParticipant)
			if err != nil {
				return nil, errors.Wrap(err, "failed to generate id")
			}

			identified, err = s.neo4j.FlowParticipantWriteRepository.Merge(ctx, txWithPostCommit.Tx, &toStore)
			if err != nil {
			...
			if flow.Status == neo4jentity.FlowStatusOn {
				err := s.flowExecution.ScheduleFlow(ctx, txWithPostCommit, flowId, entity)
				if err != nil {
				...
				txWithPostCommit.AddPostCommitAction(func(ctx context.Context) error {
					...
					err = s.flowExecution.UpdateParticipantFlowRequirements(ctx, nil, entity, flowRequirements)
					if err != nil {
						return err
					}

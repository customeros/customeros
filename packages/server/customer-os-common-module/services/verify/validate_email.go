...
func (s *verifyService) getDomainValidationWithTimeout(ctx context.Context, domain, email string) (postgresentity.CacheEmailValidationDomain, bool, error) {
...
func (s *verifyService) getDomainValidation(ctx context.Context, domain, email string) (postgresentity.CacheEmailValidationDomain, error) {
	...
	if cacheDomain == nil || cacheDomain.IsPrimaryDomain == nil || cacheDomain.UpdatedAt.AddDate(0, 0, s.cfg.EmailConfig.EmailDomainValidationCacheTtlDays).Before(utils.Now()) {
		...
		cacheDomain, err = s.postgres.CacheEmailValidationDomainRepository.Save(ctx, postgresentity.CacheEmailValidationDomain{
			Domain:              domain,
...
func (s *verifyService) getEmailValidationWithTimeout(ctx context.Context, email string, syntaxValidation mailsherpa.SyntaxValidation, isPrimaryDomain bool, primaryDomain string) (postgresentity.CacheEmailValidation, bool, error) {
	ctxWithTimeout, cancel := context.WithTimeout(ctx, MaxDurationCallMailSherpa)
	defer cancel()
	...
	case <-ctxWithTimeout.Done():
		...
		timeout = true
		emailValidation = postgresentity.CacheEmailValidation{
			Deliverable: string(EmailDeliverableStatusUndeliverable),
		}
	}
	return emailValidation, timeout, nil
}

func (s *verifyService) getEmailValidation(ctx context.Context, email string, syntaxValidation mailsherpa.SyntaxValidation, isPrimaryDomain bool, primaryDomain string) (postgresentity.CacheEmailValidation, error) {

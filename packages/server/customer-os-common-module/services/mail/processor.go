...
func (s *mailService) ProcessEmail(ctx context.Context, tenant string, rawEmailId uuid.UUID) entity.UpdateRawEmailTable {
	...
	for _, e := range emailMessageData.Participants.AllEmails {
		err := s.neo4j.EmailWriteRepository.SetDeliverableByEmailForAllTenants(ctx, e, "true")
		if err != nil {
			tracing.TraceErr(span, errors.Wrap(err, "failed to set deliverable by email for all tenants"))
		}
	}

	sentAt, err := utils.ConvertToUTC(emailMessageData.Content.SentDate)
	...
	interactionEventId, err := s.neo4j.InteractionEventReadRepository.GetInteractionEventIdByExternalId(
...
func (s *mailService) processSessionAndEvents(ctx context.Context, txWithPostCommit *utils.TxWithPostCommit, tenant string, emailMessageData *interfaces.EmailMessageData, rawEmail *postgresentity.RawEmail) error {
	...
	_, err := utils.ExecuteWriteInTransactionWithPostCommitActions(ctx, s.neo4j.Neo4jDriver, s.neo4j.Database, txWithPostCommit, func(txWithPostCommit *utils.TxWithPostCommit) (any, error) {
		...
		sessionId, err := s.neo4j.InteractionSessionWriteRepository.MergeByIdentifierAndChannel(
		...
		interactionEventId, err := s.neo4j.InteractionEventWriteRepository.MergeByExternalSystem(
		...
		if err = s.neo4j.InteractionEventWriteRepository.LinkInteractionEventToSession(
...
func (s *mailService) buildEmailForCustomerOS(email *interfaces.EmailMessageData, externalSystem string) model.SaveEmailMessage {
...
func (s *mailService) linkParticipants(ctx context.Context, txWithPostCommit *utils.TxWithPostCommit, tenant string, interactionEventId string, participants *interfaces.EmailParticipants, externalSystem string) error {
	...
	_, err := utils.ExecuteWriteInTransactionWithPostCommitActions(ctx, s.neo4j.Neo4jDriver, s.neo4j.Database, txWithPostCommit, func(txWithPostCommit *utils.TxWithPostCommit) (any, error) {
		...
		if fromEmailId != "" {
			if err = s.neo4j.InteractionEventWriteRepository.InteractionEventSentByEmail(ctx, txWithPostCommit.Tx, tenant, interactionEventId, fromEmailId); err != nil {
...
func (s *mailService) linkEmailGroup(ctx context.Context, txWithPostCommit *utils.TxWithPostCommit, tenant string, interactionEventId string, groupType string, emails []string, externalSystem string, emailIds map[string]string) error {
	...
	_, err := utils.ExecuteWriteInTransactionWithPostCommitActions(ctx, s.neo4j.Neo4jDriver, s.neo4j.Database, txWithPostCommit, func(txWithPostCommit *utils.TxWithPostCommit) (any, error) {
		...
		if len(groupEmailIds) > 0 {
			return nil, s.neo4j.InteractionEventWriteRepository.InteractionEventSentToEmails(
...
func (s *mailService) buildChannelData(email *interfaces.EmailMessageData, span opentracing.Span) error {

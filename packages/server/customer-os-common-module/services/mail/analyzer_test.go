...
func TestIsAutoResponder(t *testing.T) {
	...
	tests := []struct {
		name     string
		headers  interfaces.EmailHeaders
		wantBool bool
		wantMsg  string
	}{
		{
			name:     "X-Autoreply present",
			headers:  interfaces.EmailHeaders{XAutoreply: "yes"},
			wantBool: true,
			wantMsg:  "AUTORESPONDER | X-AUTOREPLY",
		},
		{
			name:     "X-Autoreply empty string",
			headers:  interfaces.EmailHeaders{XAutoreply: ""},
			wantBool: false,
			wantMsg:  "",
		},
		{
			name:     "X-Autoresponse present",
			headers:  interfaces.EmailHeaders{XAutoresponse: "yes"},
			wantBool: true,
			wantMsg:  "AUTORESPONDER | X-AUTORESPONSE",
		},
		{
			name:     "X-Loop present",
			headers:  interfaces.EmailHeaders{XLoop: true},
			wantBool: true,
			wantMsg:  "AUTORESPONDER | X-LOOP",
		},
		{
			name:     "Precedence auto_reply",
			headers:  interfaces.EmailHeaders{Precedence: "auto_reply"},
			wantBool: true,
...
func TestIsBounce(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			headers:  interfaces.EmailHeaders{ContentDescription: "delivery report"},
			wantBool: true,
			wantMsg:  "BOUNCE | CONTENT-DESCRIPTION: DELIVERY REPORT",
		},
		{
			name:     "Return-Path contains mailer-daemon",
			headers:  interfaces.EmailHeaders{ReturnPath: "mailer-daemon@example.com"},
			wantBool: true,
...
func TestIsBulkMail(t *testing.T) {
	...
	tests := []struct {
		...
		headers  interfaces.EmailHeaders
		from     string
		replyTo  []interfaces.EmailParticipant
		wantBool bool
		wantMsg  string
	}{
		{

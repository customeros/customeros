...
func (s *aiService) AskAIForCompanyDescription(ctx context.Context, request interfaces.AskAIRequest) (*data_fields.CompanyDescription, error) {
	...
	// defaults
	request.RequestType = enum.AIRequestCompanyDescription
...
%s`,
	...
	if lastError != nil {
		err := fmt.Errorf("company description generation failed after %d attempts: %w", *request.Retries, lastError)
		tracing.TraceErr(span, err)
		return nil, err
	}

	return nil, fmt.Errorf("company description generation failed after %d attempts", *request.Retries)
}

...
func (s *aiService) AskAIForWebpageCategory(ctx context.Context, request interfaces.AskAIRequest) (enum.WebpageCategory, error) {
	...
	for attempt := 0; attempt < *request.Retries; attempt++ {
		...
		if validator.IsValid(cleanResponse) {
			...
			category, err := validator.ParseCategory(cleanResponse)
			if err != nil {
				lastError = err
				continue
			}
		...
		// Invalid response
		lastError = fmt.Errorf("invalid webpage category value: %s", cleanResponse)
	}

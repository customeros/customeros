...
func (s *googleService) ServiceAccountCredentialsExistsForTenant(ctx context.Context, tenant string) (bool, error) {
	...
	privateKey, err := s.postgres.GoogleServiceAccountKeyRepository.GetApiKeyByTenantService(ctx, tenant, postgresEntity.GSUITE_SERVICE_PRIVATE_KEY)
	if err != nil {
		return false, nil
	}

	serviceEmail, err := s.postgres.GoogleServiceAccountKeyRepository.GetApiKeyByTenantService(ctx, tenant, postgresEntity.GSUITE_SERVICE_EMAIL_ADDRESS)
	if err != nil {
		return false, nil
	}
...
func (s *googleService) GetGmailService(ctx context.Context, username, tenant string) (*gmail.Service, error) {
	...
	if serviceAccountExistsForTenant {
		...
		tokenEntity, err := s.postgres.OAuthTokenRepository.GetByEmail(ctx, tenant, commonenum.WorkspaceProviderGoogle.String(), username)
		if err != nil {
			return nil, err
		}

...
func (a *agentVisitorIDService) SaveAgentConfig(ctx context.Context, agent *entity.Agents, agentConfig any) (*entity.Agents, error) {
	...
	if !ok {
		err := errors.New("Agent config type not supported")
		tracing.TraceErr(span, err)
		return nil, err
	}

	if agent.IsActive == true && len(config.Websites) == 0 {
	...
	// Save to db
	savedAgent, err := a.postgres.AgentsRepository.Create(ctx, agent)
	if err != nil {
		tracing.TraceErr(span, err)
		return nil, err
	}
...
func (a *agentVisitorIDService) AgentConfig(ctx context.Context, agent *entity.Agents) (any, error) {

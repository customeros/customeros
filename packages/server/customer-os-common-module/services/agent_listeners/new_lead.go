...
func (l *NewLeadListener) Handle(ctx context.Context, baseEvent any) error {
	...
	return l.handle(ctx, message, event.Event.EventType)
}

func (l *NewLeadListener) handle(ctx context.Context, message message, eventName string) error {
	span, ctx := opentracing.StartSpanFromContext(ctx, "NewLeadListener.handle")
	defer span.Finish()
	tracing.SetDefaultListenerSpanTags(ctx, span)
...
func (l *NewLeadListener) lookupActiveAgents(ctx context.Context, agentTypes []enum.AgentType) []postgres_entity.Agent {
	...
	agents, err := l.postresRepositories.AgentRepository.GetActiveConfiguredAgentsByTypes(ctx, agentTypes)
	if err != nil {
		tracing.TraceErr(span, err)
		return nil
	}

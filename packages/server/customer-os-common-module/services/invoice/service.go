...
func NewInvoiceService(log logger.Logger, grpc *grpc_client.Clients, neo4j *neoRepo.Repositories, contract interfaces.ContractService, sli interfaces.ServiceLineItemService, tenantSettings interfaces.TenantSettingsService) interfaces.InvoiceService {
...
func (s *invoiceService) GetById(ctx context.Context, invoiceId string) (*neo4jentity.InvoiceEntity, error) {
	...
	if invoiceDbNode, err := s.neo4j.InvoiceReadRepository.GetInvoiceById(ctx, common.GetTenantFromContext(ctx), invoiceId); err != nil {
...
func (h *invoiceService) FillOffCyclePrepaidInvoice(ctx context.Context, invoiceEntity *neo4jentity.InvoiceEntity, sliEntities neo4jentity.ServiceLineItemEntities) (*neo4jentity.InvoiceEntity, []*invoicepb.InvoiceLine, error) {
	...
	for parentId, slis := range sliByParentID {
		...
		if sliEntityToInvoice.Billed == neo4jenum.BilledTypeOnce {
			...
			if ilDbNodeAndInvoiceId != nil {
				previousInvoiceDbNode, err := h.neo4j.InvoiceReadRepository.GetInvoiceById(ctx, common.GetTenantFromContext(ctx), ilDbNodeAndInvoiceId.LinkedNodeId)
				if err != nil {
					tracing.TraceErr(span, err)
...
func (s *invoiceService) UpdateInvoice(ctx context.Context, invoiceId string, data neo4jrepository.InvoiceUpdateFields) error {
	...
	err = s.neo4j.InvoiceWriteRepository.UpdateInvoice(ctx, tenant, invoiceId, data)
	if err != nil {
		tracing.TraceErr(span, err)
		return err
	}
...
func (s *invoiceService) createInvoiceAction(ctx context.Context, tenant string, previousStatus neo4jenum.InvoiceStatus, invoiceEntity neo4jentity.InvoiceEntity) {
	...
	if invoiceEntity.Status == neo4jenum.InvoiceStatusDue {
		...
		_, err = s.neo4j.ActionWriteRepository.Create(ctx, tenant, invoiceEntity.Id, model.INVOICE, actionType, message, metadata, utils.Now(), common.GetAppSourceFromContext(ctx))
	}
	if err != nil {
		tracing.TraceErr(span, err)

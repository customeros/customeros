...
func (s *jobRoleService) Save(ctx context.Context, txWithPostCommit *utils.TxWithPostCommit, id, contactId, organizationId *string, dataFields data_fields.JobRoleFields) (string, error) {
	...
	if createFlow {
		...
		// generate new id
		jobRoleId, err = s.services.Neo4jRepositories.CommonReadRepository.GenerateId(ctx, tenant, model.NodeLabelJobRole)
		if err != nil {
			tracing.TraceErr(span, err)
			return "", err
		}
	...
	_, err = utils.ExecuteWriteInTransactionWithPostCommitActions(ctx, s.services.Neo4jRepositories.Neo4jDriver, s.services.Neo4jRepositories.Database, txWithPostCommit, func(txWithPostCommit *utils.TxWithPostCommit) (any, error) {
		...
		if createFlow {
			...
			if utils.IfNotNilString(organizationId) != "" {
				...
				if err = s.services.Neo4jRepositories.JobRoleWriteRepository.CreateJobRoleForContact(ctx, txWithPostCommit.Tx, common.GetContext(ctx).Tenant, jobRoleId, utils.IfNotNilString(contactId), dataFields); err != nil {
					return "", err
				}
			}
		} else {
			if err = s.services.Neo4jRepositories.JobRoleWriteRepository.UpdateJobRoleDetails(ctx, txWithPostCommit.Tx, common.GetContext(ctx).Tenant, jobRoleId, dataFields); err != nil {
				return "", err
			}
		}

		if utils.IfNotNilBool(dataFields.Primary) == true && utils.IfNotNilString(contactId) != "" {
			err = s.services.Neo4jRepositories.JobRoleWriteRepository.SetOtherJobRolesForContactNonPrimaryInTx(ctx, txWithPostCommit.Tx, common.GetContext(ctx).Tenant, utils.IfNotNilString(contactId), jobRoleId)
			if err != nil {
				return nil, err
			}
		}
		...
		txWithPostCommit.AddPostCommitAction(func(ctx context.Context) error {
			err = s.services.RabbitMQService.PublishEvent(ctx, jobRoleId, model.JOB_ROLE, dto.SaveJobRole{dataFields})
			if err != nil {
				tracing.TraceErr(span, err)
			}

			if utils.IfNotNilString(contactId) != "" {
				s.services.RabbitMQService.PublishEventCompleted(ctx, tenant, utils.IfNotNilString(contactId), model.CONTACT, utils.NewEventCompletedDetails().WithUpdate())
			}
			if utils.IfNotNilString(organizationId) != "" {
				s.services.RabbitMQService.PublishEventCompleted(ctx, tenant, utils.IfNotNilString(organizationId), model.ORGANIZATION, utils.NewEventCompletedDetails().WithUpdate())
			}
...
func (s *jobRoleService) IdentifyJobRole(ctx context.Context, contactId, organizationId string) (*neo4jentity.JobRoleEntity, error) {
	...
	var err error
	if organizationId != "" {
		jobRoleDbNode, err = s.services.Neo4jRepositories.JobRoleReadRepository.GetJobRoleForContactAndOrganization(ctx, common.GetTenantFromContext(ctx), contactId, organizationId)
	} else {
		jobRoleDbNode, err = s.services.Neo4jRepositories.JobRoleReadRepository.GetJobRoleForContactWithoutOrganization(ctx, common.GetTenantFromContext(ctx), contactId)
	}
	if err != nil {
		tracing.TraceErr(span, err)
		return nil, err
...
func (s *jobRoleService) GetById(ctx context.Context, jobRoleId string) (*neo4jentity.JobRoleEntity, error) {
	...
	dbNode, err := s.services.Neo4jRepositories.JobRoleReadRepository.GetById(ctx, common.GetTenantFromContext(ctx), jobRoleId)
	if err != nil {
		tracing.TraceErr(span, err)
		return nil, err
	}

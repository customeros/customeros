...
func addMailRoutes(conf *c.Config, rg *gin.RouterGroup, mailService *s.MailService) {
	...
	mailGroup.POST("/:id/reply/", func(c *gin.Context) {
		var interactionEventId string
		var request model.MailReplyRequest

		if err := c.ShouldBindUri(&interactionEventId); err != nil {
		...
		replyMail, err := mailService.ReplyToMail(&request, &interactionEventId, &username, &identityId)
		if err != nil {
			c.JSON(http.StatusBadRequest, gin.H{"msg": err.Error()})
			return
		}

...
// }
//
// func (s *webEventProcessor) processNewSession(ctx context.Context, message *pb.WebTrackerEvent, webtrackerID string) error {
...
// }
//
// func (s *webEventProcessor) checkForExistingIPRecord(ctx context.Context, ipAddress string) (*models.IPIntelligence, error) {
// 	span, ctx := telemetry.StartServiceSpan(ctx, "webEventProcessor.checkForExistingIPRecord")
// 	defer span.Finish()
//
...
// 	}
//
// 	cutoffDate := time.Now().AddDate(0, 0, IP_DATA_LOOKBACK_DAYS)
...
// }
//
// func (s *webEventProcessor) processNewIP(ctx context.Context, event *dto.WebTrackerEvent) (*models.IPIntelligence, error) {
// 	span, ctx := telemetry.StartServiceSpan(ctx, "webEventProcessor.processNewIP")
// 	defer span.Finish()
//
// 	// check if bot, return early if not trusted IP
// 	userAgent := utils.ParseUserAgent(event.UserAgent)
// 	ipProfile, err := s.profileIP(ctx, event.IP)
...
//
// 	// attempt to identify
// 	domain, err := s.identifyIP(ctx, event.IP)
// 	if err != nil {
// 		span.TraceError(err)
// 		return
// 	}
// }
//
// func (s *webEventProcessor) identifyIP(ctx context.Context, ipAddress string) (string, error) {
// 	span, ctx := telemetry.StartServiceSpan(ctx, "webEventProcessor.identifyIP")
// 	defer span.Finish()
//
...
// }
//
// func (s *webEventProcessor) profileIP(ctx context.Context, ipAddress string) (*pb.IPAddressVerifyResponse, error) {
// 	span, ctx := telemetry.StartServiceSpan(ctx, "webEventProcessor.isTrustedIP")
// 	defer span.Finish()
//

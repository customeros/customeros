...
func TestServiceAccountCredentialsExistsForTenant(t *testing.T) {
	tenant := "test_tenant"
	err := postgrest.InsertTenantAPIKey(postgresGormDB, tenant, postgresEntity.GSUITE_SERVICE_PRIVATE_KEY, "test1")
	require.NoError(t, err)
	err = postgrest.InsertTenantAPIKey(postgresGormDB, tenant, postgresEntity.GSUITE_SERVICE_EMAIL_ADDRESS, "test2")
	require.NoError(t, err)

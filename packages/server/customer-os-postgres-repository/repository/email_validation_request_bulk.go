...
type EmailValidationRequestBulkRepository interface {
	RegisterRequest(ctx context.Context, tenant, requestId, fileName string, totalRecords int) (*entity.EmailValidationRequestBulk, error)
...
func (r emailValidationRequestBulkRepository) RegisterRequest(ctx context.Context, tenant, requestId, fileName string, totalRecords int) (*entity.EmailValidationRequestBulk, error) {
	span, _ := opentracing.StartSpanFromContext(ctx, "EmailValidationRequestBulkRepository.RegisterRequest")
	defer span.Finish()
	tracing.TagComponentPostgresRepository(span)
	tracing.TagTenant(span, tenant)
	span.LogFields(log.String("requestId", requestId), log.String("fileName", fileName), log.Int("totalRecords", totalRecords))

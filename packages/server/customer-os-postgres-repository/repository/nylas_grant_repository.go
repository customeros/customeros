...
type NylasGrantRepository interface {
	...
	GetAllByTenant(ctx context.Context, tenant string) ([]*postgres_entity.NylasGrant, error)
...
func (r *nylasGrantRepository) GetAllByTenant(ctx context.Context, tenant string) ([]*postgres_entity.NylasGrant, error) {
	spans, _ := telemetry.StartPostgresSpan(ctx, "NylasGrantRepository.GetAllByTenant")
	defer spans.Finish()
	spans.LogKV("tenant", tenant)

	var grants []*postgres_entity.NylasGrant
	err := r.db.Where("tenant = ?", tenant).Find(&grants).Error
	if err != nil {
		spans.TraceError(err)
		return nil, err
	}

	spans.LogKV("result.count", len(grants))
	return grants, nil
}

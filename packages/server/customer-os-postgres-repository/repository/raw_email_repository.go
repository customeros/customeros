...
func (repo *rawEmailRepositoryImpl) CountForUsername(ctx context.Context, externalSystem, tenant, username string) (int64, error) {
	...
	var result int64
	err := repo.gormDb.Model(postgres_entity.RawEmail{}).Where("external_system = ? AND tenant = ? AND username = ?", externalSystem, tenant, username).Count(&result).Error
	if err != nil {
...
func (repo *rawEmailRepositoryImpl) GetByMessageId(ctx context.Context, externalSystem, tenant, username, messageId string) (*postgres_entity.RawEmail, error) {
...
func (repo *rawEmailRepositoryImpl) EmailExistsByMessageId(ctx context.Context, externalSystem, tenant, username, messageId string) (bool, error) {
	...
	var result int64
	err := repo.gormDb.Model(postgres_entity.RawEmail{}).Where("external_system = ? AND tenant = ? AND username = ? AND message_id = ?", externalSystem, tenant, username, messageId).Count(&result).Error
	if err != nil {
		tracing.TraceErr(span, err)
		return false, err
	}
...
func (repo *rawEmailRepositoryImpl) Store(ctx context.Context, externalSystem, tenant, username, providerMessageId, messageId, rawEmail string, sentAt time.Time, state postgres_entity.EmailImportState) error {

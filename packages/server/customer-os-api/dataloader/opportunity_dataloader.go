...
func (i *Loaders) GetOpportunitiesForTask(ctx context.Context, taskId string) (*neo4jentity.OpportunityEntities, error) {
	thunk := i.OpportunitiesForTask.Load(ctx, dataloader.StringKey(taskId))
	result, err := thunk()
	if err != nil {
		return nil, err
	}
...
func (b *opportunityBatcher) getOpportunitiesForTasks(ctx context.Context, keys dataloader.Keys) []*dataloader.Result {
	...
	opportunityEntitiesByTaskId := make(map[string]neo4jentity.OpportunityEntities)
	for _, val := range *opportunityEntitiesPtr {
		if list, ok := opportunityEntitiesByTaskId[val.DataloaderKey]; ok {
			opportunityEntitiesByTaskId[val.DataloaderKey] = append(list, val)
		} else {
			opportunityEntitiesByTaskId[val.DataloaderKey] = neo4jentity.OpportunityEntities{val}
		}
	}

	// construct an output array of dataloader results
	results := make([]*dataloader.Result, len(keys))
	for taskId, record := range opportunityEntitiesByTaskId {

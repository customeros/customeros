...
func (r *mutationResolver) ContactFindWorkEmail(ctx context.Context, contactID string, organizationID *string, domain *string, findMobileNumber *bool) (*model.ActionResponse, error) {
	...
	_, betterComtactRequestID, enrichmentResponse, err := r.Services.CommonServices.EnrichmentService.FindWorkEmail(ctx, linkedInUrl, firstName, lastName, orgName, *domain, false)
	if err != nil {
		tracing.TraceErr(span, err)

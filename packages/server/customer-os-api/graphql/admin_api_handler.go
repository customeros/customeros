...
func (aah *AdminApiHandler) GetAdminApiHandlerEnhancer() func(c *gin.Context) {
	return func(c *gin.Context) {
		...
		if apiKey != aah.cfg.App.Admin.Key {
		...
		if tenant != "" {
			exists, err := aah.neo4j.TenantReadRepository.TenantExists(ctx, tenant)
			if err != nil {
		...
		if usernameHeader != "" {
			userId, tenantName, roles, err := aah.neo4j.UserReadRepository.FindFirstUserWithRolesByEmail(ctx, usernameHeader)
			if err != nil {

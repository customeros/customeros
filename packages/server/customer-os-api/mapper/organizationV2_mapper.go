...
func MapEntityToOrganizationUIDetails(entity *neo4jentity.OrganizationEntity, output *model.OrganizationUIDetails) {
	...
	output.WrongIndustry = entity.WrongIndustry
	...
	output.QualificationStatus = utils.ToPtr(enummapper.MapStatusToModel(entity.QualificationStatus))
	output.QualifiedBy = utils.ToPtr(enummapper.MapQualifiedByToModel(entity.QualifiedBy))

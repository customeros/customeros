...
func (s *meetingService) Create(ctx context.Context, newMeeting *cosapi_interfaces.MeetingCreateData) (*neo4jentity.MeetingEntity, error) {
	...
	return neo4jmapper.MapDbNodeToMeetingEntity(queryResult.(*dbtype.Node)), nil
}
...
func (s *meetingService) GetMeetingById(ctx context.Context, id string) (*neo4jentity.MeetingEntity, error) {
	session := utils.NewNeo4jReadSession(ctx, s.getNeo4jDriver())
	defer session.Close(ctx)
	...
	return neo4jmapper.MapDbNodeToMeetingEntity(utils.ToPtr(queryResult.(dbtype.Node))), nil
}
...
func (s *meetingService) GetMeetingForInteractionEvent(ctx context.Context, interactionEventId string) (*neo4jentity.MeetingEntity, error) {
	...
	return neo4jmapper.MapDbNodeToMeetingEntity(record), nil
}
...
func (s *meetingService) GetMeetingsForInteractionEvents(ctx context.Context, ids []string) (*neo4jentity.MeetingEntities, error) {
	...
	meetingEntities := make(neo4jentity.MeetingEntities, 0, len(issues))
	for _, v := range issues {
		meetingEntity := neo4jmapper.MapDbNodeToMeetingEntity(v.Node)
...
func (s *meetingService) FindAll(ctx context.Context, externalSystemID string, externalID *string, page, limit int, filter *model.Filter, sortBy []*commonModel.SortBy) (*utils.Pagination, error) {
	...
	cypherSort, err := api_sort.BuildSort(sortBy, reflect.TypeOf(neo4jentity.MeetingEntity{}))
	if err != nil {
		return nil, err
	}
	cypherFilter, err := api_filters.BuildFilter(filter, reflect.TypeOf(neo4jentity.MeetingEntity{}))
	if err != nil {
		return nil, err
	}
	...
	meetings := make(neo4jentity.MeetingEntities, 0, len(dbNodesWithTotalCount.Nodes))

	for _, v := range dbNodesWithTotalCount.Nodes {
		meetings = append(meetings, *neo4jmapper.MapDbNodeToMeetingEntity(v))
	}

...
func NewJWTTenantUserService(cfg *config.FileStoreConfig) *JWTService {
	return &JWTService{
		cfg: cfg,
	}
}

func (jtus *JWTService) GetJWTTenantUserEnhancer() func(c *gin.Context) {
	return func(c *gin.Context) {
		...
		if jwtHeader == "" {
			...
			parsedToken, err := jwt.ParseWithClaims(jwtHeader, &jwt.StandardClaims{}, func(token *jwt.Token) (interface{}, error) {
				...
				return []byte(jtus.cfg.FileStoreJwtSecret), nil
			})
...
func (jtus *JWTService) MakeJWT(ctx *gin.Context) {
	...
	secretKey := []byte(jtus.cfg.FileStoreJwtSecret)

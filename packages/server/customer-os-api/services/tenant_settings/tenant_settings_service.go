...
const (
	...
	SERVICE_AMPLITUDE          = "amplitude"
	SERVICE_ASANA              = "asana"
	SERVICE_BATON              = "baton"
	SERVICE_BABELFORCE         = "babelforce"
	SERVICE_BIGQUERY           = "bigquery"
	SERVICE_BRAINTREE          = "braintree"
	SERVICE_CALLRAIL           = "callrail"
	SERVICE_CHARGEBEE          = "chargebee"
	SERVICE_CHARGIFY           = "chargify"
	SERVICE_CLICKUP            = "clickup"
	SERVICE_CLOSECOM           = "close-oauth"
	SERVICE_CODA               = "coda"
	SERVICE_CONFLUENCE         = "confluence"
	SERVICE_COURIER            = "courier"
	SERVICE_CUSTOMERIO         = "customerio"
	SERVICE_DATADOG            = "datadog"
	SERVICE_DELIGHTED          = "delighted"
	SERVICE_DIXA               = "dixa"
	SERVICE_DRIFT              = "drift"
	SERVICE_EMAILOCTOPUS       = "emailoctopus"
	SERVICE_FACEBOOK_MARKETING = "facebookMarketing"
	SERVICE_FASTBILL           = "fastbill"
	SERVICE_FLEXPORT           = "flexport"
	SERVICE_FRESHCALLER        = "freshcaller"
	SERVICE_FRESHDESK          = "freshdesk"
	SERVICE_FRESHSALES         = "freshsales"
	SERVICE_FRESHSERVICE       = "freshservice"
	SERVICE_GENESYS            = "genesys"
	SERVICE_GITHUB             = "github"
	SERVICE_GITLAB             = "gitlab"
	SERVICE_GOCARDLESS         = "gocardless"
	SERVICE_GONG               = "gong"
	SERVICE_HARVEST            = "harvest"
	SERVICE_INSIGHTLY          = "insightly"
	SERVICE_INSTAGRAM          = "instagram"
	SERVICE_INSTATUS           = "instatus"
	SERVICE_INTERCOM           = "intercom"
	SERVICE_KLAVIYO            = "klaviyo"
	SERVICE_KUSTOMER           = "kustomer"
	SERVICE_LOOKER             = "looker"
	SERVICE_MAILCHIMP          = "mailchimp"
	SERVICE_MAILJETEMAIL       = "mailjetemail"
	SERVICE_MARKETO            = "marketo"
	SERVICE_MICROSOFT_TEAMS    = "microsoftteams"
	SERVICE_MONDAY             = "monday"
	SERVICE_NOTION             = "notion"
	SERVICE_ORB                = "orb"
	SERVICE_ORACLE_NETSUITE    = "oraclenetsuite"
	SERVICE_ORBIT              = "orbit"
	SERVICE_PAGERDUTY          = "pagerduty"
	SERVICE_PAYSTACK           = "paystack"
	SERVICE_PENDO              = "pendo"
	SERVICE_PIPEDRIVE          = "pipedrive"
	SERVICE_PLAID              = "plaid"
	SERVICE_PLAUSIBLE          = "plausible"
	SERVICE_PAYPAL_TRANSACTION = "paypaltransaction"
	SERVICE_POSTHOG            = "posthog"
	SERVICE_QUALAROO           = "qualaroo"
	SERVICE_QUICKBOOKS         = "quickbooks"
	SERVICE_RECHARGE           = "recharge"
	SERVICE_RECRUITEE          = "recruitee"
	SERVICE_RECURLY            = "recurly"
	SERVICE_RETENTLY           = "retently"
	SERVICE_SALESLOFT          = "salesloft"
	SERVICE_SENDGRID           = "sendgrid"
	SERVICE_SENTRY             = "sentry"
	SERVICE_SLACK              = "slack"
	SERVICE_STRIPE             = "stripe"
	SERVICE_SURVEYSPARROW      = "surveysparrow"
	SERVICE_SURVEYMONKEY       = "surveymonkey"
	SERVICE_TALKDESK           = "talkdesk"
	SERVICE_TIKTOK             = "tiktok"
	SERVICE_TODOIST            = "todoist"
	SERVICE_TYPEFORM           = "typeform"
	SERVICE_VITTALLY           = "vittally"
	SERVICE_WRIKE              = "wrike"
	SERVICE_XERO               = "xero"
	SERVICE_ZENDESK_SUPPORT    = "zendesksupport"
	SERVICE_ZENDESK_CHAT       = "zendeskchat"
	SERVICE_ZENDESK_TALK       = "zendesktalk"
	SERVICE_ZENDESK_SELL       = "zendesksell"
	SERVICE_ZENDESK_SUNSHINE   = "zendesksunshine"
	SERVICE_ZENEFITS           = "zenefits"
...
func (s *tenantSettingsService) GetForTenant(tenantName string) (*postgresentity.TenantSettings, map[string]bool, error) {
	settings, err := s.postgres.TenantSettingsRepository.FindForTenantName(context.TODO(), tenantName)
	if err != nil {
		return nil, nil, err
	}
	...
	if settings.LinkedInCredential != nil && settings.LinkedInPassword != nil {
		credential, err1 := utils.Decrypt(*settings.LinkedInCredential, *settings.LinkedInCredentialIV, s.cfg.App.EncodedEncryptionKey)
		password, err2 := utils.Decrypt(*settings.LinkedInPassword, *settings.LinkedInPasswordIV, s.cfg.App.EncodedEncryptionKey)
...
func (s *tenantSettingsService) GetServiceActivations(tenantName string) (map[string]bool, error) {
	...
	for service, keyMappings := range s.serviceMap {
		...
		for _, mapping := range keyMappings {
			...
			keyValue, err := s.postgres.GoogleServiceAccountKeyRepository.GetApiKeyByTenantService(context.TODO(), tenantName, mapping.DbKeyName)
			if err != nil {
...
func (s *tenantSettingsService) SaveIntegrationData(tenantName string, request map[string]interface{}) (*postgresentity.TenantSettings, map[string]bool, error) {
	...
	if tenantSettings == nil {
		...
		_, err := s.postgres.TenantSettingsRepository.Save(context.TODO(), tenantSettings)
		if err != nil {
			return nil, nil, err
		}
	}
	...
	for integrationId, value := range request {
		...
		case SERVICE_LINKEDIN:
			...
			encryptedCredential, credentialIV, err := utils.Encrypt(linkedInCredential, s.cfg.App.EncodedEncryptionKey)
			if err != nil {
				return nil, nil, fmt.Errorf("SaveIntegrationData: %s", err.Error())
			}
			encryptedPassword, passwordIV, err := utils.Encrypt(linkedInPassword, s.cfg.App.EncodedEncryptionKey)
			if err != nil {
				return nil, nil, fmt.Errorf("SaveIntegrationData: %s", err.Error())
			}
	...
	if legacyUpdate {
		tenantSettings, err = s.postgres.TenantSettingsRepository.Save(context.TODO(), tenantSettings)
		if err != nil {
			return nil, nil, err
		}
...
func (s *tenantSettingsService) ClearIntegrationData(tenantName, identifier string) (*postgresentity.TenantSettings, map[string]bool, error) {
	...
	if tenantSettings == nil {
		...
		if ok {
			for _, mapping := range mappings {
				err := s.postgres.GoogleServiceAccountKeyRepository.DeleteKey(ctx, tenantName, mapping.DbKeyName)
				if err != nil {
		...
		save, err := s.postgres.TenantSettingsRepository.Save(ctx, tenantSettings)
		if err != nil {
			return nil, nil, err
		}

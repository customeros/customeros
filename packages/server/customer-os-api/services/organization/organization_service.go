...
func (s *organizationService) UpdateLastTouchpoint(ctx context.Context, organizationID string) {
	...
	_ = s.organization.RequestRefreshLastTouchpoint(ctx, organizationID)
}
...
func (s *organizationService) updateLastTouchpointByContactId(ctx context.Context, contactID string) {
	...
	for _, dbNode := range dbNodesWithTotalCount.Nodes {
		...
		if orgID != "" {
			_ = s.organization.RequestRefreshLastTouchpoint(ctx, orgID)
		}
	}
}
...
func (s *organizationService) GetOrganizationsForContact(ctx context.Context, contactId string, page, limit int, filter *model.Filter, sortBy []*commonmodel.SortBy) (*utils.Pagination, error) {
	...
	cypherSort, err := api_sort.BuildSort(sortBy, reflect.TypeOf(neo4jentity.OrganizationEntity{}))
	if err != nil {
		return nil, err
	}
	cypherFilter, err := api_filters.BuildFilter(filter, reflect.TypeOf(neo4jentity.OrganizationEntity{}))
	if err != nil {
		return nil, err
	}
...
func (s *organizationService) Merge(ctx context.Context, primaryOrganizationId, mergedOrganizationId string) error {
	...
	_, err := s.organization.GetById(ctx, tenant, primaryOrganizationId)
	if err != nil {
		s.log.Errorf("(organizationService.Merge) Primary organization with id {%s} not found: {%v}", primaryOrganizationId, err.Error())
		return err
	}
	_, err = s.organization.GetById(ctx, tenant, mergedOrganizationId)
	if err != nil {

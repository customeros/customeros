...
func registerPublicRoutes(ctx context.Context, r *gin.Engine, services *service.Services) {
	...
	setupPublicRoute(ctx, r, services, "POST", fmt.Sprintf("%s/:tenantId/i/:integrationId", flowsV1Path), flows.HandleWebhook(services, flowsV1Path))
}
...
func registerFlowRoutes(ctx context.Context, r *gin.Engine, services *service.Services, cache *commoncaches.Cache) {
	setupRestRoute(ctx, r, "GET", fmt.Sprintf("%s/events", flowsV1Path), services, cache, flows.GetEvents(services))

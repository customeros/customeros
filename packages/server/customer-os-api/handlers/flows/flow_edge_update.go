...
func UpdateFlowEdge(s *service.Services) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		switch edgeId {
		case "":
			handlers.SendError(c, span, http.StatusBadRequest, enum.ErrBadRequest.WithMessage("missing nodeID"))
			return
		default:
...
func updateFlowEdge(c *gin.Context, s *service.Services, updateRecord FlowEdgeRecord) {
	...
	if edge == nil {
		handlers.SendError(c, span, http.StatusNotFound, enum.ErrNotFound.WithMessage("edge not found"))
		return
	}

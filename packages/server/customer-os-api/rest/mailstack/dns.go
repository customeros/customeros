...
func DNS(s *service.Services) gin.HandlerFunc {
    ...
    // validate domain belongs to tenant
		...
		if err != nil {
			rest.SendError(c, span, http.StatusInternalServerError, enum.ErrInternalServer.WithMessage("unable to validate domain ownership"))
			return
		}
...
func AddDNSRecord(s *service.Services) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err != nil {
			rest.SendError(c, span, http.StatusInternalServerError, enum.ErrInternalServer.WithMessage("unable to verify domain"))
			return
		}
		if !domainExists {
			rest.SendError(c, span, http.StatusNotFound, enum.ErrNotFound.WithMessage("domain not found"))
		...
		if err != nil {
			rest.SendError(c, span, http.StatusBadRequest, enum.ErrBadRequest.WithMessage("unable to parse request"))
			return
		}
		err = s.CommonServices.CloudflareService.AddDNSRecord(ctx, zoneId, record.Type, record.Name, record.Content, 1, false, nil)
		if err != nil {
			rest.SendError(c, span, http.StatusInternalServerError, enum.ErrInternalServer.WithMessage("unable to create DNS record"))
			return
		}
...
func DeleteDNSRecord(s *service.Services) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err != nil {
			rest.SendError(c, span, http.StatusInternalServerError, enum.ErrInternalServer.WithMessage("unable to delete dns record"))
			return
		}

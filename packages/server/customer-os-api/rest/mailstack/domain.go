...
func registerDomain(ctx context.Context, tenant, domain, website string, services *cosapi_services.Services) (DomainRecord, error) {
	...
	tldSupported := false
	for _, supportedTld := range services.Cfg.CommonServices.InternalServices.MailstackConfig.SupportedTlds {
		if tld == supportedTld {
			tldSupported = true
			break
		}
	...
	if domainPrice > services.Cfg.CommonServices.ExternalServices.NamecheapConfig.MaxPrice {

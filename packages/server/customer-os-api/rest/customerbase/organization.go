...
func GetOrganization(s *service.Services) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		orgID := c.Param("id")
		if orgID == "" {
			rest.SendError(c, span, http.StatusBadRequest, rest.ErrBadRequest.WithMessage("Invalid organization ID b"))
			return
		}
		...
		case status == rest.StatusError:
			rest.SendError(c, span, http.StatusNotFound, rest.ErrNotFound.WithMessage("Organization does not exist b"))
			return

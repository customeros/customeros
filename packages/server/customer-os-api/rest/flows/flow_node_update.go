	...
	return func(c *gin.Context) {
		...
		default:
			...
			if err != nil {
				...
				rest.SendError(c, span, http.StatusBadRequest, enum.ErrBadRequest.WithMessage("unable to parse requst"))
				return
			}
...
func updateFlowNode(c *gin.Context, s *service.Services, updateRecord FlowNodeRecord) {
	...
	if node == nil {
		rest.SendError(c, span, http.StatusNotFound, enum.ErrNotFound.WithMessage("node not found"))
		return
	}

...
func getTenant(c context.Context, services *cosapi_services.Services, personalEmailProvider []entity.PersonalEmailProvider, signInRequest SignInRequest, config *config.Config) (*string, bool, error) {
	...
	if config.CommonServices.ExternalServices.SlackConfig.NotifyNewTenantRegisteredHook != "" {
		common_utils.SendSlackMessage(ctx, config.CommonServices.ExternalServices.SlackConfig.NotifyNewTenantRegisteredHook, tenantStr+" tenant registered by "+signInRequest.LoggedInEmail)
	}
...
func getUserInfoFromGoogle(c context.Context, config *config.Config, signInRequest SignInRequest) (*googleOauth.Userinfo, error) {
	...
	conf := &tokenOauth.Config{
		ClientID:     config.CommonServices.GoogleOAuthConfig.ClientId,
		ClientSecret: config.CommonServices.GoogleOAuthConfig.ClientSecret,

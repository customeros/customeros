	...
	return func(c *gin.Context) {
		...
		c.JSON(http.StatusOK,
			BulkUploadResponse{
				...
				JobID:                 requestID,
				ResultURL:             fmt.Sprintf("%s/verify/v1/email/bulk/results/%s", services.Cfg.CommonServices.InternalServices.CustomerOsApiUrl, requestID), // Placeholder for results URL
	...
	return func(c *gin.Context) {
		...
		c.JSON(http.StatusOK, BulkResultsResponse{
			...
			Results: &BulkResultsDetails{
				...
				DownloadURL:   fmt.Sprintf("%s/verify/v1/email/bulk/results/%s/download", services.Cfg.CommonServices.InternalServices.CustomerOsApiUrl, requestID),
			},
		})
	}
}

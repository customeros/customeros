...
func TestQueryResolver_GetNote_WithMentionedEntities(t *testing.T) {
	ctx := context.TODO()
	defer tearDownTestCase(ctx)(t)

	neo4jt.CreateTenant(ctx, driver, tenantName)
	organizationId := neo4jt.CreateOrganization(ctx, driver, tenantName, "test org")
	noteId := neo4jt.CreateNoteForOrganization(ctx, driver, tenantName, organizationId, "Note content", utils.Now())

	issueId := neo4jt.CreateIssue(ctx, driver, tenantName, entity.IssueEntity{
		Subject:   "testSubject",
		CreatedAt: utils.Now(),
	})
	tagId := neo4jt.CreateTag(ctx, driver, tenantName, "issue-tag")
	neo4jt.TagIssue(ctx, driver, issueId, tagId)
	neo4jt.NoteMentionsTag(ctx, driver, noteId, tagId)

	rawResponse := callGraphQL(t, "note/get_note_with_mentioned_entities_via_organization_query", map[string]interface{}{"organizationId": organizationId})
	...
	var mentionedIssue interface{}

	if note.(map[string]interface{})["mentioned"].([]interface{})[0].(map[string]interface{})["__typename"] == "Issue" {
		mentionedIssue = note.(map[string]interface{})["mentioned"].([]interface{})[0]
	}

	require.Equal(t, "Issue", mentionedIssue.(map[string]interface{})["__typename"])

	require.Equal(t, issueId, mentionedIssue.(map[string]interface{})["id"])
	require.Equal(t, "testSubject", mentionedIssue.(map[string]interface{})["subject"])
	...
	require.Equal(t, 1, neo4jt.GetCountOfRelationships(ctx, driver, "NOTED"))
	require.Equal(t, 1, neo4jt.GetCountOfRelationships(ctx, driver, "MENTIONED"))
	require.Equal(t, 1, neo4jt.GetCountOfRelationships(ctx, driver, "TAGGED"))

	// Check the labels on the nodes in the Neo4j database
	assertNeo4jLabels(ctx, t, driver, []string{"Tenant", "Issue", "Issue_" + tenantName,
		"Organization", "Organization_" + tenantName, "Note", "Note_" + tenantName,
		"TimelineEvent", "TimelineEvent_" + tenantName, "Tag"})
}

...
//func TestMutationResolver_MeetingCreate_Min(t *testing.T) {
//	ctx := context.TODO()
//	defer tearDownTestCase(ctx)(t)
//	neo4jt.CreateTenant(ctx, driver, tenantName)
//	neo4jt.CreateDefaultUserWithId(ctx, driver, tenantName, testUserId)
//
//	rawResponse, err := c.RawPost(getQuery("meeting/create_meeting_min"))
...
//	require.Nil(t, err)
//	require.Equal(t, "Oasis", meeting.Meeting_Create.AppSource)
//}

//func TestMutationResolver_MeetingCreateWithAttachment(t *testing.T) {
//	ctx := context.TODO()
//	defer tearDownTestCase(ctx)(t)
//	neo4jt.CreateTenant(ctx, driver, tenantName)
//	neo4jt.CreateDefaultUserWithId(ctx, driver, tenantName, testUserId)
...
//	now := time.Now().UTC()
//
//	meeting1 := neo4jt.CreateMeeting(ctx, driver, tenantName, "mySessionIdentifier", "session1", "THREAD", "ACTIVE", "EMAIL", now, false)
...
//	err = decode.Decode(rawResponse.Data.(map[string]interface{}), &meeting)
//	require.Nil(t, err)
//	require.Equal(t, "ACTIVE", meeting.Meeting_LinkAttachment.Status)
//	require.Equal(t, "EMAIL", *meeting.Meeting_LinkAttachment.Channel)
//	require.Equal(t, "test", meeting.Meeting_LinkAttachment.AppSource)
//	require.Len(t, meeting.Meeting_LinkAttachment.Includes, 1)
//	require.Equal(t, attachmentId, meeting.Meeting_LinkAttachment.Includes[0].ID)
//	require.Equal(t, "text/plain", meeting.Meeting_LinkAttachment.Includes[0].MimeType)
//}
//
//func TestMutationResolver_MeetingCreateWithPhone(t *testing.T) {
//	ctx := context.TODO()
//	defer tearDownTestCase(ctx)(t)
//	neo4jt.CreateTenant(ctx, driver, tenantName)
//	neo4jt.CreateDefaultUserWithId(ctx, driver, tenantName, testUserId)
...
//
//	rawResponse, err := c.RawPost(getQuery("meeting/create_meeting_with_phone"),
...
//	err = decode.Decode(rawResponse.Data.(map[string]interface{}), &meeting)
//	require.Nil(t, err)
//	require.Equal(t, "ACTIVE", meeting.Meeting_Create.Status)
//	require.Equal(t, "EMAIL", meeting.Meeting_Create.Channel)
//	require.Equal(t, "{\"threading-info\":\"test\"}", meeting.Meeting_Create.ChannelData)
//	require.Equal(t, "Oasis", meeting.Meeting_Create.AppSource)
//	require.Equal(t, "My Session Identifier", meeting.Meeting_Create.SessionIdentifier)
//	require.Equal(t, "My Session Name", meeting.Meeting_Create.Name)
//
//	for _, attendedBy := range meeting.Meeting_Create.AttendedBy {
...
//}
//
//func TestMutationResolver_MeetingCreate(t *testing.T) {
//	ctx := context.TODO()
//	defer tearDownTestCase(ctx)(t)
//	neo4jt.CreateTenant(ctx, driver, tenantName)
//	neo4jt.CreateDefaultUserWithId(ctx, driver, tenantName, testUserId)
//
//	rawResponse, err := c.RawPost(getQuery("meeting/create_meeting"),
//		client.Var("sessionIdentifier", "My Session Identifier"),
//		client.Var("name", "My Session Name"),
//		client.Var("type", "THREAD"),
//		client.Var("channel", "EMAIL"),
...
//	meeting1 := neo4jt.CreateMeeting(ctx, driver, tenantName, "mySessionIdentifier", "session1", "THREAD", "ACTIVE", "EMAIL", now, false)
//
//	rawResponse, err := c.RawPost(getQuery("meeting/create_meeting_email"),
...
//
//	require.Equal(t, firstEvent.InteractionEvent_Create.Meeting.ID, meeting1)
//	require.Equal(t, firstEvent.InteractionEvent_Create.Meeting.Name, "session1")
//
...
//	now := time.Now().UTC()
//
//	meeting1 := neo4jt.CreateMeeting(ctx, driver, tenantName, "mySessionIdentifier", "session1", "THREAD", "ACTIVE", "VOICE", now, false)
//
//	rawResponse, err := c.RawPost(getQuery("meeting/create_meeting_call"),
//		client.Var("content", "Content 1"),
//		client.Var("contentType", "text/plain"),
//		client.Var("sessionId", meeting1))
//	assertRawResponseSuccess(t, rawResponse, err)
...
//
//	require.Equal(t, interactionEvent.InteractionEvent_Create.Meeting.ID, meeting1)
//	require.Equal(t, interactionEvent.InteractionEvent_Create.Meeting.Name, "session1")
//
//	rawResponse, err = c.RawPost(getQuery("meeting/create_meeting_call2"),
//		client.Var("content", "Content 2"),
//		client.Var("contentType", "text/plain"),
//		client.Var("sessionId", meeting1))
//	assertRawResponseSuccess(t, rawResponse, err)
...
//	meeting1 := neo4jt.CreateMeeting(ctx, driver, tenantName, "mySessionIdentifier", "session1", "THREAD", "ACTIVE", "EMAIL", now, false)
//
//	neo4jt.InteractionEventPartOfMeeting(ctx, driver, interactionEventId1, meeting1)
...
//	rawResponse, err := c.RawPost(getQuery("meeting/get_meeting"),
//		client.Var("eventId", interactionEventId1))
//	assertRawResponseSuccess(t, rawResponse, err)
...
//
//	require.Equal(t, meeting1, timelineEvent1["meeting"].(map[string]interface{})["id"].(string))
//	require.Equal(t, "session1", timelineEvent1["meeting"].(map[string]interface{})["name"].(string))
//	require.Equal(t, "THREAD", timelineEvent1["meeting"].(map[string]interface{})["type"].(string))
//	require.Equal(t, "ACTIVE", timelineEvent1["meeting"].(map[string]interface{})["status"].(string))
//	require.Equal(t, "EMAIL", timelineEvent1["meeting"].(map[string]interface{})["channel"].(string))
//	require.Equal(t, "OPENLINE", timelineEvent1["meeting"].(map[string]interface{})["source"].(string))
//	require.Equal(t, "OPENLINE", timelineEvent1["meeting"].(map[string]interface{})["sourceOfTruth"].(string))
//	require.Equal(t, "test", timelineEvent1["meeting"].(map[string]interface{})["appSource"].(string))
//	require.NotNil(t, timelineEvent1["meeting"].(map[string]interface{})["startedAt"].(string))
//	require.NotNil(t, timelineEvent1["meeting"].(map[string]interface{})["endedAt"].(string))
//
//	rawResponse, err = c.RawPost(getQuery("meeting/get_meeting"),
...
//	neo4jt.InteractionEventPartOfMeeting(ctx, driver, interactionEventId1, meeting1)
//	neo4jt.InteractionEventRepliesToInteractionEvent(ctx, driver, tenantName, interactionEventId1, interactionEventId4_WithoutSession)
//	rawResponse, err := c.RawPost(getQuery("meeting/get_meeting_by_identifier"),
...
//}
//
//func TestQueryResolver_Meeting(t *testing.T) {
//	ctx := context.TODO()
//	defer tearDownTestCase(ctx)(t)
//	neo4jt.CreateTenant(ctx, driver, tenantName)
//
...
//		t.Fatalf("response is not a map")
//	}
//	interactionEventInterface := responseMap["meeting"]
//	timelineEvent1, ok := interactionEventInterface.(map[string]interface{})
//	if !ok {
//		t.Fatalf("timelineEventInterface is not a map")
//	}
//
//	require.Equal(t, meeting1, timelineEvent1["id"].(string))
...
//}
//
//func TestQueryResolver_Meeting_BySessionIdentifier(t *testing.T) {
//	ctx := context.TODO()
//	defer tearDownTestCase(ctx)(t)
//	neo4jt.CreateTenant(ctx, driver, tenantName)
//
...
//		t.Fatalf("response is not a map")
//	}
//	interactionEventInterface := responseMap["meeting_BySessionIdentifier"]
//	timelineEvent1, ok := interactionEventInterface.(map[string]interface{})
//	if !ok {
//		t.Fatalf("timelineEventInterface is not a map")
//	}
...
//}
//
//func TestQueryResolver_Contact_WithTimelineEvents_InteractionEvents_With_Meeting(t *testing.T) {
//	ctx := context.TODO()
//	defer tearDownTestCase(ctx)(t)
//	neo4jt.CreateTenant(ctx, driver, tenantName)
//
...
//	neo4jt.InteractionEventSentTo(ctx, driver, interactionEventId4_WithoutSession, emailId, "")
//
//	meeting1 := neo4jt.CreateMeeting(ctx, driver, tenantName, "mySessionIdentifier", "session1", "THREAD", "ACTIVE", "EMAIL", now, true)
//	meeting2 := neo4jt.CreateMeeting(ctx, driver, tenantName, "mySessionIdentifier", "session2", "THREAD", "INACTIVE", "EMAIL", now, true)
//
//	neo4jt.InteractionEventPartOfMeeting(ctx, driver, interactionEventId1, meeting1)
//	neo4jt.InteractionEventPartOfMeeting(ctx, driver, interactionEventId2, meeting2)
//	neo4jt.InteractionEventPartOfMeeting(ctx, driver, interactionEventId3, meeting2)
//
...
//
//	rawResponse, err := c.RawPost(getQuery("meeting/get_meetings_with_session_in_timeline_event"),
...
//	require.Equal(t, meeting2, timelineEvent2["meeting"].(map[string]interface{})["id"].(string))
//	require.Equal(t, "session2", timelineEvent2["meeting"].(map[string]interface{})["name"].(string))
//	require.Equal(t, "THREAD", timelineEvent2["meeting"].(map[string]interface{})["type"].(string))
//	require.Equal(t, "INACTIVE", timelineEvent2["meeting"].(map[string]interface{})["status"].(string))
//	require.Equal(t, "EMAIL", timelineEvent2["meeting"].(map[string]interface{})["channel"].(string))
//	require.Equal(t, "OPENLINE", timelineEvent2["meeting"].(map[string]interface{})["source"].(string))
//	require.Equal(t, "OPENLINE", timelineEvent2["meeting"].(map[string]interface{})["sourceOfTruth"].(string))
//	require.Equal(t, "test", timelineEvent2["meeting"].(map[string]interface{})["appSource"].(string))
//	require.NotNil(t, timelineEvent2["meeting"].(map[string]interface{})["startedAt"].(string))
//	require.NotNil(t, timelineEvent2["meeting"].(map[string]interface{})["endedAt"].(string))
//
...
//	require.Equal(t, meeting2, timelineEvent3["meeting"].(map[string]interface{})["id"].(string))
//
...
//	require.Equal(t, "OPENLINE", timelineEvent4["sourceOfTruth"].(string))
//	require.Equal(t, "test", timelineEvent4["appSource"].(string))
//	require.Nil(t, timelineEvent4["meeting"])
//}
//
...
//
//	rawResponse, err := c.RawPost(getQuery("meeting/get_meetings_with_participants_in_timeline_event"),
//		client.Var("contactId", contactId),
//		client.Var("from", now),
//		client.Var("size", 100))
//	assertRawResponseSuccess(t, rawResponse, err)

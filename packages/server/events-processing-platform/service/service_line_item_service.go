...
func (s *serviceLineItemService) UpdateServiceLineItemSet(ctx context.Context, request *servicelineitempb.UpdateServiceLineItemSetGrpcRequest) (*servicelineitempb.ServiceLineItemIdSetGrpcResponse, error) {
	ctx, span := tracing.StartGrpcServerTracerSpan(ctx, "ServiceLineItemService.UpdateServiceLineItemSet")
	defer span.Finish()
	tracing.SetServiceSpanTags(ctx, span, request.Tenant, request.LoggedInUserId)
	span.LogFields(log.Object("request", request))

	// Validate service line item set
	if request.ServiceLineItems == nil {
	...
	for _, item := range request.ServiceLineItems {
		if item.Id == "" {
			...
			createCommand := command.NewCreateServiceLineItemCommand(
				...
				request.LoggedInUserId,
				model.ServiceLineItemDataFields{
					Billed:     model.BilledType(item.Billed),
	...
	return &servicelineitempb.ServiceLineItemIdSetGrpcResponse{Ids: allIds}, nil
}

...
import (
	...
	utils2 "github.com/customeros/customeros/packages/server/events/constants"
	...
	postgresentity "github.com/customeros/customeros/packages/server/events-processing-platform/repository/postgres/entity"
...
func NewInvoiceService(repositories *repository.Repositories, log logger.Logger, aggregateStore eventstore.AggregateStore, req RequestHandler) *invoiceService {
...
func (s *invoiceService) NextPreviewInvoiceForContract(ctx context.Context, request *invoicepb.NextPreviewInvoiceForContractRequest) (*invoicepb.InvoiceIdResponse, error) {
	...
	if _, err := s.requestHandlerService.HandleGRPCRequest(ctx, initAggregateFunc, eventstore.LoadAggregateOptions{}, &invoicepb.NewInvoiceForContractRequest{
		...
		InvoicePeriodStart:   request.InvoicePeriodStart,
		InvoicePeriodEnd:     request.InvoicePeriodEnd,
...
func (s *invoiceService) getContract2(ctx context.Context, tenant, contractId string) (*neo4jentity.ContractEntity, error) {
...
func (s *invoiceService) NewInvoiceForContract(ctx context.Context, request *invoicepb.NewInvoiceForContractRequest) (*invoicepb.InvoiceIdResponse, error) {
	...
	if _, err := s.requestHandlerService.HandleGRPCRequest(ctx, initAggregateFunc, eventstore.LoadAggregateOptions{}, request); err != nil {
		tracing.TraceErr(span, err)
		s.log.Errorf("(NewOnCycleInvoiceForContract) tenant:{%v}, err: %v", request.Tenant, err.Error())
		return nil, grpcerr.ErrResponse(err)
	}
...
func (s *invoiceService) FillInvoice(ctx context.Context, request *invoicepb.FillInvoiceRequest) (*invoicepb.InvoiceIdResponse, error) {
	...
	if request.InvoiceNumber != "" {
		...
		if !request.DryRun || request.Preview {
			...
			extraParams[invoice.PARAM_INVOICE_NUMBER] = s.generateNewRandomInvoiceNumber()
		}
	}
	...
	if _, err := s.requestHandlerService.HandleGRPCRequest(ctx, initAggregateFunc, *eventstore.NewLoadAggregateOptionsWithRequired(), request, extraParams); err != nil {
		tracing.TraceErr(span, err)

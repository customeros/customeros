...
type CloseEndedContractCommandHandler interface {
	Handle(ctx context.Context, cmd *command.CloseEndedContractCommand) error
}

type closeEndedContractCommandHandler struct {
	log logger.Logger
	es  eventstore.AggregateStore
	cfg config.Utils
}

func NewCloseEndedContractCommandHandler(log logger.Logger, es eventstore.AggregateStore, cfg config.Utils) CloseEndedContractCommandHandler {
	return &closeEndedContractCommandHandler{
		log: log,
		es:  es,
		cfg: cfg,
	}
...
func (h *closeEndedContractCommandHandler) Handle(ctx context.Context, cmd *command.CloseEndedContractCommand) error {
	span, ctx := opentracing.StartSpanFromContext(ctx, "CloseEndedContractCommandHandler.Handle")
	defer span.Finish()

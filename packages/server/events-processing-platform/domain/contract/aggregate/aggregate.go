...
func (a *ContractAggregate) When(evt eventstore.Event) error {
	...
	case event.ContractCloseEndedV1:
		return a.onContractCloseEnded(evt)
	case event.ContractResetRenewalOpportunityV1:
		return nil
	default:
...
func (a *ContractAggregate) onContractCloseEnded(evt eventstore.Event) error {
	var eventData event.EndedContractCloseEvent
	if err := evt.GetJsonData(&eventData); err != nil {
		return errors.Wrap(err, "GetJsonData")
	}

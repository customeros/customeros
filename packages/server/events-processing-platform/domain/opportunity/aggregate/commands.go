...
func (a *OpportunityAggregate) updateRenewalOpportunity(ctx context.Context, cmd *command.UpdateRenewalOpportunityCommand) error {
	...
	updateRenewalEvent, err := event.NewOpportunityUpdateRenewalEvent(a, string(renewalLikelihood), cmd.Comments, cmd.LoggedInUserId, cmd.Source.Source, cmd.Amount, cmd.OwnerUserId, updatedAtNotNil)
	if err != nil {
		tracing.TraceErr(span, err)

...
func (c *phoneNumberValidatedCommandHandler) Handle(ctx context.Context, command *PhoneNumberValidatedCommand) error {
	...
	if err = phoneNumberAggregate.PhoneNumberValidated(ctx, command.Tenant, command.RawPhoneNumber, command.ValidationError, command.Domain, command.Username, command.NormalizedPhoneNumber,

...
type updateMasterPlanMilestoneCommandHandler struct {
	log logger.Logger
...
func NewUpdateMasterPlanMilestoneCommandHandler(log logger.Logger, es eventstore.AggregateStore, cfg config.Utils) UpdateMasterPlanMilestoneCommandHandler {
	return &updateMasterPlanMilestoneCommandHandler{log: log, es: es, cfg: cfg}
}

// Handle processes the UpdateMasterPlanCommand to update a new master plan.
func (h *updateMasterPlanMilestoneCommandHandler) Handle(ctx context.Context, cmd *command.UpdateMasterPlanMilestoneCommand) error {
	span, ctx := opentracing.StartSpanFromContext(ctx, "updateMasterPlanMilestoneCommandHandler.Handle")
	defer span.Finish()

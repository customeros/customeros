...
type reorderMasterPlanMilestonesCommandHandler struct {
	log logger.Logger
...
func NewReorderMasterPlanMilestonesCommandHandler(log logger.Logger, es eventstore.AggregateStore, cfg config.Utils) ReorderMasterPlanMilestonesCommandHandler {
	return &reorderMasterPlanMilestonesCommandHandler{log: log, es: es, cfg: cfg}
}

func (h *reorderMasterPlanMilestonesCommandHandler) Handle(ctx context.Context, cmd *command.ReorderMasterPlanMilestonesCommand) error {
	span, ctx := opentracing.StartSpanFromContext(ctx, "reorderMasterPlanMilestonesCommandHandler.Handle")
	defer span.Finish()

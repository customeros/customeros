...
type updateOnboardingStatusCommandHandler struct {
	log logger.Logger
...
func NewUpdateOnboardingStatusCommandHandler(log logger.Logger, es eventstore.AggregateStore, cfg config.Utils) UpdateOnboardingStatusCommandHandler {
	return &updateOnboardingStatusCommandHandler{log: log, es: es, cfg: cfg}
}

func (h *updateOnboardingStatusCommandHandler) Handle(ctx context.Context, cmd *command.UpdateOnboardingStatusCommand) error {
	span, ctx := opentracing.StartSpanFromContext(ctx, "UpdateOnboardingStatusCommandHandler.Handle")
	defer span.Finish()

...
func validateEmailV2(ctx context.Context, r *gin.Engine, services *service.Services, cfg *config.Config, l logger.Logger) {
	r.POST("/validateEmailV2",
		...
		security.ApiKeyCheckerHTTP(services.Postgres.TenantWebhookApiKeyRepository, services.Postgres.AppKeyRepository, security.VALIDATION_API, security.WithCache(services.Cache)),
		func(c *gin.Context) {

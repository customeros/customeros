...
func DispatchWebhook(ctx context.Context, tenant string, event WebhookEvent, payload *InvoicePayload, postgresRepositories *postgresRepository.Repositories, cfg config.Config) error {
	...
	if !cfg.CommonServices.ExternalServices.TemporalConfig.RunWorker {
	...
	tClient, err := temporal_client.TemporalClient(
		cfg.CommonServices.ExternalServices.TemporalConfig.HostPort,
		cfg.CommonServices.ExternalServices.TemporalConfig.Namespace)
	if err != nil {
		tracing.TraceErr(span, err)
	...
	if cfg.CommonServices.ExternalServices.TemporalConfig.NotifyOnFailure {
	...
	workflowParams := workflows.WHWorkflowParam{
		...
		NotificationProviderApiKey: cfg.CommonServices.ExternalServices.NovuCofig.ApiKey,
		NotifyFailure:              cfg.CommonServices.ExternalServices.TemporalConfig.NotifyOnFailure,
		NotifyAfterAttempts:        cfg.CommonServices.ExternalServices.TemporalConfig.NotifyAfterAttempts,
	}

...
export const SuccessPage = observer(() => {
  ...
  useEffect(() => {
    if (store.sessionStore.isHydrated && store.sessionStore.isAuthenticated) {
      const originPath = new URLSearchParams(window.location.search).get(
        'origin',
      );

      store.sessionStore.fetchSession({
        onSuccess: () =>
          setTimeout(() => navigate(originPath ?? '/organizations'), 500),
      });
    }
  }, [store.sessionStore.isHydrated, store.sessionStore.isAuthenticated]);
  ...
  return (
    <div
      className={cn(
        ...
        store.sessionStore.isBootstrapped && 'opacity-0',
      )}
    >

...
export type TenantBillingProfilesQueryVariables = Types.Exact<{
  [key: string]: never;
}>;

export type TenantBillingProfilesQuery = {
  __typename?: 'Query';
  tenantBillingProfiles: Array<{
...
>(
  ...
  options?: Omit<
    UseQueryOptions<TenantBillingProfilesQuery, TError, TData>,
    'queryKey'
  > & {
    queryKey?: UseQueryOptions<
      TenantBillingProfilesQuery,
      TError,
      TData
    >['queryKey'];
  },
  ...
  return useQuery<TenantBillingProfilesQuery, TError, TData>({
    queryKey:
      variables === undefined
        ? ['TenantBillingProfiles']
        : ['TenantBillingProfiles', variables],
    queryFn: fetcher<
      TenantBillingProfilesQuery,
      TenantBillingProfilesQueryVariables
    >(client, TenantBillingProfilesDocument, variables, headers),
    ...options,
  });
};
...
useTenantBillingProfilesQuery.getKey = (
  variables?: TenantBillingProfilesQueryVariables,
) =>
  variables === undefined
    ? ['TenantBillingProfiles']
...
>(
  ...
  variables: TenantBillingProfilesQueryVariables,
  options: Omit<
    UseInfiniteQueryOptions<TenantBillingProfilesQuery, TError, TData>,
    'queryKey'
  > & {
      ...
      return {
        queryKey:
          optionsQueryKey ?? variables === undefined
            ? ['TenantBillingProfiles.infinite']
            : ['TenantBillingProfiles.infinite', variables],
        queryFn: (metaData) =>
          fetcher<
            TenantBillingProfilesQuery,
            TenantBillingProfilesQueryVariables
...
useInfiniteTenantBillingProfilesQuery.getKey = (
  variables?: TenantBillingProfilesQueryVariables,
) =>
  variables === undefined
    ? ['TenantBillingProfiles.infinite']
...
useTenantBillingProfilesQuery.fetcher = (
  client: GraphQLClient,
  variables?: TenantBillingProfilesQueryVariables,
  headers?: RequestInit['headers'],
) =>
  fetcher<TenantBillingProfilesQuery, TenantBillingProfilesQueryVariables>(
    client,
    TenantBillingProfilesDocument,
    variables,
    headers,
...
useTenantBillingProfilesQuery.mutateCacheEntry =
  ...
  (
    mutator: (
      cacheEntry: TenantBillingProfilesQuery,
    ) => TenantBillingProfilesQuery,
  ) => {
...
useInfiniteTenantBillingProfilesQuery.mutateCacheEntry =
  ...
  (
    mutator: (
      cacheEntry: InfiniteData<TenantBillingProfilesQuery>,
    ) => InfiniteData<TenantBillingProfilesQuery>,
  ) => {
    const cacheKey = useInfiniteTenantBillingProfilesQuery.getKey(variables);
    const previousEntries =
      queryClient.getQueryData<InfiniteData<TenantBillingProfilesQuery>>(
        cacheKey,
      );
    if (previousEntries) {
      queryClient.setQueryData<InfiniteData<TenantBillingProfilesQuery>>(
        cacheKey,
        mutator,
      );
    }

...
>(
  ...
  options?: Omit<
    UseQueryOptions<BankAccountsQuery, TError, TData>,
    'queryKey'
  > & {
    queryKey?: UseQueryOptions<BankAccountsQuery, TError, TData>['queryKey'];
  },
  headers?: RequestInit['headers'],
) => {
  return useQuery<BankAccountsQuery, TError, TData>({
    queryKey:
      variables === undefined ? ['BankAccounts'] : ['BankAccounts', variables],
    queryFn: fetcher<BankAccountsQuery, BankAccountsQueryVariables>(
      client,
...
useBankAccountsQuery.getKey = (variables?: BankAccountsQueryVariables) =>
  variables === undefined ? ['BankAccounts'] : ['BankAccounts', variables];
...
>(
  ...
  variables: BankAccountsQueryVariables,
  options: Omit<
    UseInfiniteQueryOptions<BankAccountsQuery, TError, TData>,
    'queryKey'
  > & {
      ...
      return {
        queryKey:
          optionsQueryKey ?? variables === undefined
            ? ['BankAccounts.infinite']
            : ['BankAccounts.infinite', variables],
        queryFn: (metaData) =>
          fetcher<BankAccountsQuery, BankAccountsQueryVariables>(
            client,
            BankAccountsDocument,
...
useInfiniteBankAccountsQuery.getKey = (
  variables?: BankAccountsQueryVariables,
) =>
  variables === undefined
    ? ['BankAccounts.infinite']
...
useInfiniteBankAccountsQuery.mutateCacheEntry =
  ...
  (
    mutator: (
      cacheEntry: InfiniteData<BankAccountsQuery>,
    ) => InfiniteData<BankAccountsQuery>,
  ) => {
    ...
    if (previousEntries) {
      queryClient.setQueryData<InfiniteData<BankAccountsQuery>>(
        cacheKey,
        mutator,
      );
    }

...
export type TenantBillingProfileQuery = {
  __typename?: 'Query';
...
>(
  ...
  options?: Omit<
    UseQueryOptions<TenantBillingProfileQuery, TError, TData>,
    'queryKey'
  > & {
    queryKey?: UseQueryOptions<
      TenantBillingProfileQuery,
      TError,
      TData
    >['queryKey'];
  },
  ...
  return useQuery<TenantBillingProfileQuery, TError, TData>({
    ...
    queryFn: fetcher<
      TenantBillingProfileQuery,
      TenantBillingProfileQueryVariables
    >(client, TenantBillingProfileDocument, variables, headers),
    ...options,
  });
};
...
useTenantBillingProfileQuery.getKey = (
  variables: TenantBillingProfileQueryVariables,
) => ['TenantBillingProfile', variables];
...
>(
  ...
  variables: TenantBillingProfileQueryVariables,
  options: Omit<
    UseInfiniteQueryOptions<TenantBillingProfileQuery, TError, TData>,
    'queryKey'
  > & {
...
useInfiniteTenantBillingProfileQuery.getKey = (
  variables: TenantBillingProfileQueryVariables,
) => ['TenantBillingProfile.infinite', variables];

useTenantBillingProfileQuery.fetcher = (
  client: GraphQLClient,
  variables: TenantBillingProfileQueryVariables,
  headers?: RequestInit['headers'],
) =>
  fetcher<TenantBillingProfileQuery, TenantBillingProfileQueryVariables>(
    client,
    TenantBillingProfileDocument,
    variables,
    headers,
...
useTenantBillingProfileQuery.mutateCacheEntry =
  ...
  (
    mutator: (
      cacheEntry: TenantBillingProfileQuery,
    ) => TenantBillingProfileQuery,
  ) => {
...
useInfiniteTenantBillingProfileQuery.mutateCacheEntry =
  ...
  (
    mutator: (
      cacheEntry: InfiniteData<TenantBillingProfileQuery>,
    ) => InfiniteData<TenantBillingProfileQuery>,
  ) => {
    const cacheKey = useInfiniteTenantBillingProfileQuery.getKey(variables);
    const previousEntries =
      queryClient.getQueryData<InfiniteData<TenantBillingProfileQuery>>(
        cacheKey,
      );
    if (previousEntries) {
      queryClient.setQueryData<InfiniteData<TenantBillingProfileQuery>>(
        cacheKey,
        mutator,
      );
    }

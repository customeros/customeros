...
>(
  ...
  options?: Omit<
    UseQueryOptions<TenantSettingsQuery, TError, TData>,
    'queryKey'
  > & {
    queryKey?: UseQueryOptions<TenantSettingsQuery, TError, TData>['queryKey'];
  },
  ...
  return useQuery<TenantSettingsQuery, TError, TData>({
    queryKey:
      variables === undefined
        ? ['TenantSettings']
        : ['TenantSettings', variables],
    queryFn: fetcher<TenantSettingsQuery, TenantSettingsQueryVariables>(
      client,
...
useTenantSettingsQuery.getKey = (variables?: TenantSettingsQueryVariables) =>
  variables === undefined ? ['TenantSettings'] : ['TenantSettings', variables];
...
>(
  ...
  variables: TenantSettingsQueryVariables,
  options: Omit<
    UseInfiniteQueryOptions<TenantSettingsQuery, TError, TData>,
    'queryKey'
  > & {
      ...
      return {
        queryKey:
          optionsQueryKey ?? variables === undefined
            ? ['TenantSettings.infinite']
            : ['TenantSettings.infinite', variables],
        queryFn: (metaData) =>
          fetcher<TenantSettingsQuery, TenantSettingsQueryVariables>(
            client,
            TenantSettingsDocument,
...
useInfiniteTenantSettingsQuery.getKey = (
  variables?: TenantSettingsQueryVariables,
) =>
  variables === undefined
    ? ['TenantSettings.infinite']
...
useTenantSettingsQuery.fetcher = (
  client: GraphQLClient,
  variables?: TenantSettingsQueryVariables,
  headers?: RequestInit['headers'],
) =>
...
useInfiniteTenantSettingsQuery.mutateCacheEntry =
  ...
  (
    mutator: (
      cacheEntry: InfiniteData<TenantSettingsQuery>,
    ) => InfiniteData<TenantSettingsQuery>,
  ) => {
    ...
    if (previousEntries) {
      queryClient.setQueryData<InfiniteData<TenantSettingsQuery>>(
        cacheKey,
        mutator,
      );
    }
